...
YOLO_SEG_PATH = os.path.join(LOCAL_MODEL_DIR, "yolov8n-seg.pt")
YOLO_POSE_PATH = os.path.join(LOCAL_MODEL_DIR, "yolov8mn-pose.pt")

# Object Storage keys for YOLO models
OBJ_STORAGE_SEG_KEY = "models/yolov8n-seg.pt"
OBJ_STORAGE_POSE_KEY = "models/yolov8mn-pose.pt"


def _get_object_storage_client():
    """Return an Object Storage client, or None if unavailable."""
    try:
        return ObjectStorageClient()
    except Exception as e:
        print(f"  -> Object Storage unavailable: {e}")
        return None


def _load_yolo_model_from_storage(storage_client, storage_key: str, dest_path: str) -> bool:
    """Try to download a YOLO model from Object Storage to dest_path.
    
    Returns True if the model was successfully fetched, False otherwise.
    """
    try:
        if storage_client.exists(storage_key):
            print(f"  -> Fetching {storage_key} from Object Storage ...")
            os.makedirs(os.path.dirname(dest_path), exist_ok=True)
            storage_client.download_to_filename(storage_key, dest_path)
            print(f"  -> Loaded {dest_path} from Object Storage.")
            return True
    except Exception as e:
        print(f"  -> Could not fetch {storage_key} from Object Storage: {e}")
    return False


def _upload_yolo_model_to_storage(storage_client, storage_key: str, src_path: str):
    """Upload a local YOLO model file to Object Storage for future cold starts."""
    try:
        print(f"  -> Uploading {src_path} to Object Storage as {storage_key} ...")
        storage_client.upload_from_filename(storage_key, src_path)
        print(f"  -> Uploaded {storage_key} to Object Storage.")
    except Exception as e:
        print(f"  -> Could not upload {storage_key} to Object Storage: {e}")


def _download_yolo_model(model_name: str, dest_path: str):
    """Download a YOLO model via ultralytics (fetches from CDN) and save to dest_path."""
    print(f"  -> Downloading {model_name} from CDN to {dest_path} ...")
    os.makedirs(os.path.dirname(dest_path), exist_ok=True)
    # Instantiating YOLO with a plain model name triggers ultralytics' auto-download
    # into its default cache. We then copy the file to our persistent models dir.
    tmp_model = YOLO(model_name)
    # ultralytics saves the file relative to the cwd or its own cache; find it.
    default_path = model_name  # ultralytics places it in cwd on first run
    if os.path.exists(default_path) and not os.path.exists(dest_path):
        shutil.move(default_path, dest_path)
        print(f"  -> Saved {model_name} to {dest_path}")
    elif not os.path.exists(dest_path):
        # Already in ultralytics cache — load from there and re-export
        tmp_model.save(dest_path)
        print(f"  -> Saved {model_name} to {dest_path}")
    return tmp_model


def _ensure_yolo_models():
    """Load YOLO models directly from the local disk."""
    ...
    try:
        print(f"  -> Loading {YOLO_SEG_PATH} from local disk.")
        yolo_seg_model = YOLO(YOLO_SEG_PATH)
        
        print(f"  -> Loading {YOLO_POSE_PATH} from local disk.")
        yolo_pose_model = YOLO(YOLO_POSE_PATH)
